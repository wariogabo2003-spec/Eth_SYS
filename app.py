...
def run_full_simulation(params, prices):
    ...
    horas_año = 8000
    inversion_inicial = 200000 # Inversión ajustada para mayor sensibilidad
    utilidad_anual = (ingresos_hr - total_opex_hr) * horas_año
    
    roi = (utilidad_anual / inversion_inicial) * 100
    # Payback corregido: si la utilidad es <= 0, el tiempo es infinito (no se recupera)
    payback = inversion_inicial / utilidad_anual if utilidad_anual > 0 else 999
    npv = -inversion_inicial + (utilidad_anual / 0.12) # VAN con tasa del 12%
...
try:
    ...
    # Mostrar Payback de forma amigable
    ...
    with tab_tutor:
        st.write("Consulta al experto sobre cómo volver rentable esta planta.")
        for msg in st.session_state.messages:
            with st.chat_message(msg["role"]):
                st.markdown(msg["content"])

        if prompt := st.chat_input("¿Cómo puedo hacer que el NPV sea positivo?"):
            ...
            with st.chat_message("assistant"):
                try:
                    ...
                    context = f"""
                    Eres un ingeniero experto en BioSTEAM. Contexto técnico:
                    - ROI: {econ['ROI']:.2f}%, NPV: {econ['NPV']:.2f}
                    - Costo producción: {econ['CostoProd']:.2f} USD/kg. Precio Etanol: {p_etanol} USD/kg.
                    - Parámetros: Alimento {t_feed}C, W220 {t_w220}C, Flash {p_v100}Pa.
                    Responde de forma profesional, analizando si el problema es de proceso o de mercado.
                    """
